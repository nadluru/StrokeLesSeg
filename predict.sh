...
# res u-net model
python $NNUNet_FOLDER"inference/predict_simple.py" -i $NNUNet_DATA_FOLDER -o predictions/t100_res_fold0 -tr nnUNetTrainerV2_ResencUNet_MSCSA_Depth_1_DA3 -p nnUNetPlans_FabiansResUNet_v2.1 --num_threads_preprocessing $THREADS --num_threads_nifti_save $THREADS -t $TASK100_NAME -m 3d_fullres -f 0 -z
python $NNUNet_FOLDER"inference/predict_simple.py" -i $NNUNet_DATA_FOLDER -o predictions/t100_res_fold1 -tr nnUNetTrainerV2_ResencUNet_MSCSA_Depth_1_DA3 -p nnUNetPlans_FabiansResUNet_v2.1 --num_threads_preprocessing $THREADS --num_threads_nifti_save $THREADS -t $TASK100_NAME -m 3d_fullres -f 1 -z
python $NNUNet_FOLDER"inference/predict_simple.py" -i $NNUNet_DATA_FOLDER -o predictions/t100_res_fold2 -tr nnUNetTrainerV2_ResencUNet_MSCSA_Depth_1_DA3 -p nnUNetPlans_FabiansResUNet_v2.1 --num_threads_preprocessing $THREADS --num_threads_nifti_save $THREADS -t $TASK100_NAME -m 3d_fullres -f 2 -z
