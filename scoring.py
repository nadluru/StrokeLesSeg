...
def volume_difference(truth, prediction, batchwise=False):
    ...
    '''
    if(not batchwise):
        pred_vol = np.sum(prediction).astype(np.int32)
        truth_vol = np.sum(truth).astype(np.int32)
...
def precision(truth, prediction, batchwise=False):
    ...
    if(not batchwise):
        ...
        return precision_score(truth.reshape((num_pred,)), prediction.reshape((num_pred,)), zero_division=1.0)
    else:
...
def _recall(truth, prediction, batchwise=False, pos_label=1):
    ...
    if(not batchwise):
        # Convert to vector
        num_pred = np.prod(prediction.shape)
        return recall_score(truth.reshape((num_pred,)), prediction.reshape((num_pred,)), pos_label=pos_label, zero_division=1.0)
    else:
        # Need to get the precision for each sample in the batch
        recall_list = []
        num_pred = np.prod(prediction.shape[1:])
...
def _lesion_f1_score(truth, prediction, empty_value=1.0):
    ...
    """
    tp, fp, fn, tp_pred = 0, 0, 0, 0
    tp_separeted = [0 for _ in range(4)]
    fp_separeted = [0 for _ in range(4)]
    fn_separeted = [0 for _ in range(4)]
    tp_pred_separeted = [0 for _ in range(4)]
    ...
    for idx_lesion in range(1, num_lesions+1):
        ...
        if(np.max(lesion_pred_sum) > 1):
            tp += 1
            tp_separeted[idx_volume] += 1
        else:
            fn += 1
            fn_separeted[idx_volume] += 1
    ...
    for idx_lesion in range(1, num_pred_lesions+1):
        ...
        if(np.max(lesion_pred_sum) <= 1):  # No overlap
            fp += 1
            fp_separeted[idx_volume] += 1
        else:
            tp_pred += 1
            tp_pred_separeted[idx_volume] += 1
    ...
    denom_precision = tp_pred + fp
    precision = tp_pred / denom_precision if denom_precision != 0 else empty_value

    denom_recall = tp + fn
    recall = tp / denom_recall if denom_recall != 0 else empty_value

    results.append(f1_score)
    results.append(precision)
    results.append(recall)

    for idx_volume in range(4):
        tp = tp_separeted[idx_volume]
        fp = fp_separeted[idx_volume]
        fn = fn_separeted[idx_volume]
        tp_pred = tp_pred_separeted[idx_volume]

        denom = tp + (fp + fn)/2
        f1_score = tp / denom if denom != 0 else empty_value
