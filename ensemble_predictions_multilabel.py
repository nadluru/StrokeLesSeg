...
def merge_softmax(f):
    softmax = np.load(f)['softmax']
    bg_softmax = softmax[0:1]
    fg_softmax = softmax[1:].sum(0, keepdims=True)
    softmax = np.vstack([bg_softmax, fg_softmax])[None]
...
#     softmax = np.load(f)['softmax']
#     bg_softmax = softmax[0:1]
#     fg_softmax = softmax[1:].max(0, keepdims=True)
#     softmax = np.vstack([bg_softmax, fg_softmax])
#     softmax /= softmax.sum(0, keepdims=True)
#     softmax = softmax[None]
#     return softmax
...
def merge_files(files, properties_files, out_file, override, store_npz):
    if override or not isfile(out_file):
        softmax = [merge_softmax(f) for f in files]
