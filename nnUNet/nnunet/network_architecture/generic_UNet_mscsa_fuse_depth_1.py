...
class Generic_UNet_MSCSA(Generic_UNet):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        for i in range(len(channels)):
            self.SIM.append(
                inj_module(channels[-(i + 1)], mscsa_channels[-(i + 1)], self.mscsa_params))

        # self.SIM.append(
        #     inj_module(mscsa_channels[-1], channels[-1], self.mscsa_params))
        # for i in range(len(channels) - 1):
        #     self.SIM.append(
        #         inj_module(mscsa_channels[-(i + 2)], self.conv_blocks_localization[i][-1].output_channels, self.mscsa_params))
    ...
    def forward(self, x):
        ...
        # feat = self.forward_inject(outs, feats)

        x = self.SIM[0](outs[-1], feats[-1])

        for u in range(len(self.tu)):
            x = self.tu[u](x)
            x = torch.cat((x, outs[-(u + 2)]), dim=1)
            x = self.conv_blocks_localization[u](x)
            x = self.SIM[u + 1](x, feats[-(u + 2)])
