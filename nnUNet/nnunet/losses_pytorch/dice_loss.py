...
import torch
from nnunet.losses_pytorch.ND_Crossentropy import CrossentropyND, TopKLoss, WeightedCrossEntropyLoss
...
class GDiceLoss(nn.Module):
    ...
    def forward(self, net_output, gt):
        ...
        y_flatten = y_onehot.flatten(start_dim=-3)
        net_flatten = net_output.flatten(start_dim=-3)
        y_sum = y_flatten.sum(-1)
        net_sum = net_flatten.sum(-1)
        w = 1 / (y_sum + 1e-10)**2
        intersection = w * (net_flatten * y_flatten).sum(-1)
        union = w * (net_sum + y_sum)
        divided =  - 2 * (intersection.sum(-1) + self.smooth) / (union.sum(-1) + self.smooth)
        gdc = divided.mean()
        return gdc
